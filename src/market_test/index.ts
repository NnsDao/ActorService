export default ({ IDL }) => {
  ...
  const MarketService = IDL.Record({
    ...
    nft_project_list: IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(IDL.Tuple(Token, NftInfo)))),
  });
  ...
  const Result_5 = IDL.Variant({
    Ok: IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(IDL.Tuple(IDL.Nat32, IDL.Opt(Listing), IDL.Opt(IDL.Nat8))))),
    Err: CommonError,
  });
  ...
  return IDL.Service({
    add_nft_project: IDL.Func([IDL.Text, IDL.Vec(IDL.Tuple(Token, NftInfo))], [], []),
    ...
    balance: IDL.Func([Price, IDL.Principal, IDL.Opt(IDL.Vec(IDL.Nat8))], [Result_1], []),
    decode_token: IDL.Func([IDL.Text], [Result_2], ['query']),
    delete_nft_project: IDL.Func([IDL.Text], [], []),
    delist: IDL.Func([IDL.Text, IDL.Opt(IDL.Vec(IDL.Nat8)), IDL.Text], [Result], []),
    encode_token: IDL.Func([IDL.Principal, IDL.Nat32], [IDL.Text], ['query']),
    get_canister_log: IDL.Func([GetLogMessagesParameters], [CanisterLogMessages], ['query']),
    get_nft: IDL.Func([IDL.Text], [IDL.Opt(NFT)], ['query']),
    get_nft_project: IDL.Func([IDL.Text], [IDL.Opt(IDL.Vec(IDL.Tuple(Token, NftInfo)))], ['query']),
    get_owner: IDL.Func([], [IDL.Vec(IDL.Principal)], ['query']),
    handle_disbursement: IDL.Func([Disbursement], [Result_3], []),
    handle_failed_disbursements: IDL.Func([], [IDL.Opt(Disbursement), Result_3], []),
    list: IDL.Func([IDL.Text, IDL.Opt(IDL.Vec(IDL.Nat8)), IDL.Text, Price], [Result_4], []),
    listings: IDL.Func([IDL.Text], [IDL.Vec(IDL.Tuple(IDL.Nat32, Listing))], ['query']),
    lock: IDL.Func([IDL.Text, IDL.Text, Price, IDL.Principal, IDL.Opt(IDL.Vec(IDL.Nat8))], [Result_4], []),
    market_tokens_ext: IDL.Func([IDL.Text, IDL.Opt(IDL.Text)], [Result_5], ['query']),
    restore_disburse: IDL.Func([DisburseService], [], ['query']),
    restore_market: IDL.Func([MarketService], [], ['query']),
    return_back: IDL.Func([IDL.Text, IDL.Text, IDL.Opt(IDL.Text)], [Result_4], []),
    settle: IDL.Func([IDL.Text, IDL.Text], [Result], []),
    stats: IDL.Func([IDL.Text], [IDL.Vec(IDL.Tuple(IDL.Text, Stats))], ['query']),
    tokens_ext: IDL.Func([IDL.Text, IDL.Text], [Result_6], []),
    transfer: IDL.Func(
      [Price, IDL.Principal, IDL.Opt(IDL.Vec(IDL.Nat8)), IDL.Principal, IDL.Opt(IDL.Vec(IDL.Nat8)), Price, IDL.Text],
      [Result_7],
      []
    ),
  });
